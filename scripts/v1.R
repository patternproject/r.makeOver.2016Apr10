...
df.2 <-
  ...
  mutate(ageD = df.1$Billionaire.Age - df.1$Millionaire.Age,
         ...
         gI = rank(-ageD,ties.method= "max"), #year from millionaire to billionaire
         wI = rank(-df.1$Net.Worth,ties.method= "max")) # networth Index
...
g1 <- g1 + geom_bar(stat="identity", width=0.75, size=0.05)

# faceting
g2 <- g1 + facet_wrap(~metric, scales="free")
g2 <- g2 + labs(x=NULL, y=NULL, title="4 Metrics To Analyze The Rich")
